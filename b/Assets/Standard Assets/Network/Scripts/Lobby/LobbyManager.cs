...
namespace Prototype.NetworkLobby
{
    public class LobbyManager : NetworkLobbyManager 
    {
        ...
        // CUSTOM function to spawn selected hero instead of playerPrefab
        public override GameObject OnLobbyServerCreateGamePlayer(NetworkConnection conn, short playerControllerId) {
            // find the lobbyPlayer object
            var player = lobbySlots.ToList().Find(lp => lp.connectionToClient == conn);
            if (player != null) {
                // get the hero index
                int hero = ((LobbyPlayer)player).heroIndex;

                // spawn it
                var heroes = NetworkManager.singleton.spawnPrefabs.Where(p => p.tag == "Player").ToList();
                if (0 <= hero && hero < heroes.Count) {
                    print("spawning selected hero: " + heroes[hero].name);
                    return (GameObject)Instantiate(heroes[hero].gameObject);
                } else Debug.LogError("invalid hero index: " + hero + " of " + heroes.Count);
            } else Debug.LogError("no lobby player found for connection: " + conn);
